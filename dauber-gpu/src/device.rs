...
use wgpu::{
    BindGroupLayoutDescriptor, BufferDescriptor, BufferUsages, DeviceDescriptor, Features,
    IndexFormat, InstanceDescriptor, Limits, PowerPreference, RequestAdapterOptions,
...
pub struct Wgpu {
    ...
    surface_desc: wgpu::TextureDescriptor<'static>,
...
impl Wgpu {
    ...
    pub fn encode_to_png(&self) -> Vec<u8> {
        let mut v = Vec::new();
        {
            ...
            buffer.write_to(&mut buf, image::ImageOutputFormat::Png).unwrap();
        }
...
impl Device for Wgpu {
    fn new(info: ImageInfo) -> Self {
        ...
        let bglayout = device.create_bind_group_layout(&BindGroupLayoutDescriptor {
            label: None,
        ...
        match paint.style() {
            PaintStyle::Fill => {
                ...
                tessellation::FillTessellator::new()
                    .tessellate(
                        iter,
                        &tessellation::FillOptions::even_odd(),
                        &mut b,
                    )
                    .unwrap();
            }
        ...
        encoder.copy_texture_to_buffer(
            ...
            wgpu::ImageCopyBuffer {
                ...
                layout: wgpu::ImageDataLayout {
                    offset: 0,
                    bytes_per_row: Some(U32_SIZE * self.info.width),
                    rows_per_image: Some(self.info.height),
                },
            },
            self.surface_desc.size,
        );
